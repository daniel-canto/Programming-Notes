...
> [!NOTE] Atenção!
> Os códigos presentes nessa anotação foram escritos em Python para fins educacionais.


### O que é?
~={green}É um algoritmo eficiente para encontrar um elemento em uma lista ordenada=~. Seu funcionamento é baseado no paradigma de "dividir e conquistar": a cada passo, o algoritmo compara o elemento do meio da lista com o valor procurado e, dependendo do resultado, descarta metade dos elementos, reduzindo drasticamente o número de comparações necessárias.

----
### Funcionamento
1. O algoritmo começa definindo dois índices, que seriam **baixo** e **alto**, que representam os índices **inicial** e **final** respectivamente. Normalmente, o baixo recebe 0 (pois está começando do índice 0) enquanto o alto começa com o tamanho da lista -1 (`alto = len(lista) -1)`.

2. Calcula o índice do elemento do meio. `meio = (baixo + alto) / 2`.

3. Compara o valor do meio com o valor procurado:
	1. Se for igual, a busca termina com sucesso.
	2. Se o valor do meio for maior, a busca continua apenas na metade inferior da lista. Alterando a variável **baixo** dessa forma: `baixo = meio + 1`. 
	3. Se o valor do meio for menor, a busca continua apenas na metade superior. Alterando a variável **alto** dessa forma: `alto = meio -1`.
...
2. **Primeira Iteração: comparação com o elemento do meio**
       ...
       $$ meio =  \frac{baixo + alto}{2} = \frac{0 + 7}{2} = 3$$
	 O elemento no índice `[3]` é o elemento `7`. Como `7 < 9` , o algoritmo descarta a metade inferior da lista (elementos menores ou iguais a 7) e atualiza o limite inferior (baixo):
	 $$baixo = meio + 1 = 4$$
